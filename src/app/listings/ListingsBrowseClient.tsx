...
export type ListingsBrowseInitial = {
  listings: Listing[];
  count: number;
  numPages: number;
  city?: string;
...
export default function ListingsBrowseClient({
  initial,
}: {
  initial: ListingsBrowseInitial;
}) {
  const router = useRouter();
  const [listings, setListings] = useState<Listing[]>(initial.listings);
  const [count, setCount] = useState(initial.count);
  const [numPages, setNumPages] = useState(initial.numPages);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState(initial.status || "A");
  ...
  const [viewMode, setViewMode] = useState<"list" | "map">(initial.view || "list");
  const [mlsInput, setMlsInput] = useState("");
  const [mlsSearch, setMlsSearch] = useState("");
  const [stateFilter, setStateFilter] = useState(initial.state || "WA");
  const [totalDbCount, setTotalDbCount] = useState<number | null>(
    initial.city ? null : initial.count || null
  );
  const [dataRefreshedAt, setDataRefreshedAt] = useState<Date | null>(() => new Date());
  ...
  const skipInitialFetch = useRef(initial.listings.length > 0 || initial.count > 0);
            ...
            <div className="mb-6 flex flex-col gap-5 sm:mb-10 sm:gap-8 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="max-w-xl text-[14px] leading-7 text-white/70 sm:text-[16px] sm:leading-8">
                  Search East Pierce homes on the MLS — Lake Tapps, Bonney Lake, Sumner, Puyallup, and nearby. Filter by city, price, beds, and more.
                </p>
                <Link
                  href="/service-areas"
