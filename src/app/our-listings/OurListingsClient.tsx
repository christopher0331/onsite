...
export default function OurListingsClient({
  initial,
}: {
  initial: ApiResponse;
}) {
  const [listings, setListings] = useState<OnsiteListing[]>(initial.listings ?? []);
  const [count, setCount] = useState(initial.count ?? 0);
  const [timberCount, setTimberCount] = useState(initial.timberCount ?? 0);
  ...
  const [scope, setScope] = useState<OnsiteListingScope>(initial.scope ?? "all");
  const [status, setStatus] = useState("All");
  const skipInitialFetch = useRef(
    (initial.listings?.length ?? 0) > 0 || (initial.count ?? 0) > 0
  );

  function selectScope(next: OnsiteListingScope) {
    setScope(next);
    setPage(1);
  }
            ...
            <div className="flex w-max max-w-none items-center gap-2 sm:w-auto sm:flex-wrap">
              ...
              {SCOPE_FILTERS.map((filter) => (
                <button
                  key={filter.value}
                  type="button"
                  onClick={() => selectScope(filter.value)}
